...
=================
  ...
  - `γ`: Planetary vorticity polar coefficient.
  ...
  - `nμ`: radiation cooling order.
  - `dt`: Time-step.
  - `lagtime`: Red noise time lag.
  ...
  - `selcor`: chose γ or β or other kind of Coriolis term
"""
function Problem(dev::Device=CPU();
  ...
  # the grid
  grid = TwoDGrid(dev; nx, Lx, ny, Ly, aliased_fraction, T, nlayers=1)
  x, y = gridpoints(grid)

  # topographic PV
  eta === nothing && (eta = zeros(dev, T, (nx, ny)))

  params = deformation_radius == Inf ? BarotropicQGParams(grid, T(β), T(γ), T(selcor), eta, T(μ), T(μ0), nμ, T(ν), nν, T(lagtime), calcF) : EquivalentBarotropicQGParams(grid, T(β), T(γ), T(selcor), T(deformation_radius), eta, T(μ), T(μ0), nμ, T(ν), nν, T(lagtime), calcF)
...
"""
    EquivalentBarotropicQGParams(grid, β, γ, deformation_radius, eta, μ, nμ, ν, nν, lagtime, calcF)

Return the parameters for an Equivalent Barotropic QG problem (i.e., with finite Rossby radius of deformation).
"""
function EquivalentBarotropicQGParams(grid::AbstractGrid{T, A}, β, γ, selcor, deformation_radius, eta, μ, μ0, nμ, ν, nν::Int, lagtime, calcF) where {T, A}
  eta_on_grid = typeof(eta) <: AbstractArray ? A(eta) : FourierFlows.on_grid(eta, grid)
  etah_on_grid = rfft(eta_on_grid)

  return Params(β, γ, selcor, deformation_radius, eta_on_grid, etah_on_grid, μ, μ0, nμ, ν, nν, lagtime, calcF)
end

"""
    BarotropicQGParams(grid, β, γ, eta, μ, ν, nν, lagtime, calcF)

Return the parameters for a Barotropic QG problem (i.e., with infinite Rossby radius of deformation).
"""
BarotropicQGParams(grid::AbstractGrid, β, γ, selcor, eta, μ, μ0, nμ, ν, nν::Int, lagtime, calcF) =
    EquivalentBarotropicQGParams(grid, β, γ, selcor, nothing, eta, μ, μ0, nμ, ν, nν, lagtime, calcF)
...
#  L = @. - params.μ * grid.Krsq * (grid.Krsq + 1 / params.deformation_radius^2 + (grid.Krsq==0))^(-1) * grid.Krsq^params.nμ - params.ν * grid.Krsq^params.nν
...
#  L = @. - params.μ * grid.Krsq * (grid.Krsq + 1 / params.deformation_radius^2 + (grid.Krsq==0))^(params.nμ - 1) - params.ν * grid.Krsq^params.nν + im * params.β * grid.kr / (grid.Krsq + 1 / params.deformation_radius^2)
  L = @. - params.μ * (grid.Krsq + 1 / params.deformation_radius^2 + (grid.Krsq==0))^params.nμ - params.ν * grid.Krsq^params.nν + im * params.β * grid.kr / (grid.Krsq + 1 / params.deformation_radius^2)
#  L = @. - params.μ - params.ν * grid.Krsq^params.nν
  CUDA.@allowscalar L[1, 1] = 0
...
function DecayingVars(grid::AbstractGrid)
  ...
  @devzeros Dev Complex{T} (grid.nkr, grid.nl) qh uh vh uq_plus_ηh vq_plus_ηh mu_qh ψh

  Vars(q, ψ, u, v, qh, ψh, uh, vh, uq_plus_ηh, vq_plus_ηh, mu_qh, nothing, nothing)
end
...
function ForcedVars(grid::AbstractGrid)
  ...
  @devzeros Dev T (grid.nx, grid.ny) q u v ψ
  @devzeros Dev Complex{T} (grid.nkr, grid.nl) qh uh vh uq_plus_ηh vq_plus_ηh mu_qh ψh Fh

  return Vars(q, ψ, u, v, qh, ψh, uh, vh, uq_plus_ηh, vq_plus_ηh, mu_qh, Fh, nothing)
end
...
function StochasticForcedVars(grid::AbstractGrid)
  ...
  @devzeros Dev T (grid.nx, grid.ny) q u v ψ
  @devzeros Dev Complex{T} (grid.nkr, grid.nl) qh uh vh uq_plus_ηh vq_plus_ηh mu_qh ψh Fh prevsol

  return Vars(q, ψ, u, v, qh, ψh, uh, vh, uq_plus_ηh, vq_plus_ηh, mu_qh, Fh, prevsol)
end
...
function calcN_advection!(N, sol, t, clock, vars, params, grid)
  ...
  uqplusη = vars.u                                            # use vars.u as scratch variable
  @. uqplusη *= vars.q + params.eta - 0.5 * params.γ * grid.r2                         # u * (q + η)
  vqplusη = vars.v                                            # use vars.v as scratch variable
  @. vqplusη *= vars.q + params.eta - 0.5 * params.γ * grid.r2                          # v * (q + η)
  muq = vars.q
  @. muq *= grid.Sponge * params.μ0 * 1000
                                                                # use vars.uh as scratch variable # use vars.vh as scratch variable
  mul!(vars.uq_plus_ηh, grid.rfftplan, uqplusη)                    # \hat{u * (q + η)}
  mul!(vars.vq_plus_ηh, grid.rfftplan, vqplusη)                    # \hat{v * (q + η)}
  mul!(vars.mu_qh, grid.rfftplan, muq)
 
  @. N = -im * grid.kr * vars.uq_plus_ηh - im * grid.l * vars.vq_plus_ηh - vars.mu_qh              # - ∂[u*(q+η)]/∂x - ∂[v*(q+η)]/∂y - sponge
  #@. N = -im * grid.kr * vars.uq_plus_ηh - im * grid.l * vars.vq_plus_ηh
