...
$(TYPEDFIELDS)
"""
struct TwoDGrid{T<:AbstractFloat, Tk, Tx, Tfft, Trfft, Talias, Tgamma, Tsponge, D} <: AbstractGrid{T, Tk, Talias, D}
    "device which the grid lives on"
           device :: D
    "number of points in ``x``"
               nx :: Int
...
nonlinearities. 
"""
function TwoDGrid(dev::Device=CPU(); nx, Lx, ny=nx, Ly=Lx, nlayers,
                  x0=-Lx/2, y0=-Ly/2, nthreads=Sys.CPU_THREADS, effort=FFTW.MEASURE,
                  T=Float64, aliased_fraction=1/3)
  ...
  #if nlayers > 1
  #  aa = π
  #  r2 = device_array(dev){T}(zeros(nx,ny,nlayers))
  #  for i in 1:nx
  #    for j in 1:ny
  #      rr=(x[i]^2+y[j]^2)^0.5
  #      for k in 1:nlayers
  #        if rr<aa
  #          CUDA.@allowscalar r2[j,i,k] = rr^2
  #        else
  #          CUDA.@allowscalar r2[j,i,k] = aa^2
  #        end
  #      end
  #    end
  #  end
  #
  #  rr(xx,yy,zz) = ((xx-1/2-nx/2)^2+(yy-1/2-ny/2)^2)^0.5-aa/π*nx/2
  #  f(xx,yy,zz) = 0.5*(1+(exp(rr(xx,yy,zz))-exp(-rr(xx,yy,zz)))/(exp(rr(xx,yy,zz))+exp(-rr(xx,yy,zz))))
  #  xrange = 1:nx; yrange = 1:ny; zrange = 1:nlayers
  #  Sponge = device_array(dev){T}([f(xx,yy,zz) for xx in xrange, yy in yrange, zz in zrange])
  #
  #else
  
  aa = 2.7
  r2 = device_array(dev){T}(zeros(nx,ny))
  for i in 1:nx
    for j in 1:ny
      rr=(x[i]^2+y[j]^2)^0.5
      if rr<aa
        CUDA.@allowscalar r2[j,i] = rr^2
      else
        CUDA.@allowscalar r2[j,i] = aa^2
      end
    end
  end
    
  rr(xx,yy) = ((xx-1/2-nx/2)^2+(yy-1/2-ny/2)^2)^0.5-aa/π*nx/2
  f(xx,yy) = 0.5*(1 + (exp(min(rr(xx,yy),512)) - exp(min(-rr(xx,yy),512)))/(exp(min(rr(xx,yy),512)) + exp(min(-rr(xx,yy),512))))
  xrange = 1:nx; yrange = 1:ny
  Sponge = device_array(dev){T}([f(xx,yy) for xx in xrange, yy in yrange])
  ...
  Talias = typeof(kalias)
  Tgamma = typeof(r2)
  Tsponge = typeof(Sponge)
  D = typeof(dev)

  return TwoDGrid{T, A, R, Tfft, Trfft, Talias, Tgamma, Tsponge, D}(dev, nx, ny, nk, nl, nkr, nlayers, dx, dy, Lx, Ly, x, y, k, l, kr, 
                                                   Ksq, invKsq, Krsq, invKrsq, fftplan, rfftplan,
                                                   aliased_fraction, kalias, kralias, lalias, r2, Sponge)
end
